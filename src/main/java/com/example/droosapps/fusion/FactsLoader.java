...
import com.example.droosapps.model.BagTag;
...
public class FactsLoader
{
  ...
  /**
   * Layout of a StockTick line has to be {uuid}, {timestamp}, {symbol},
   * {price}.
   * 
   * @param line
   *          the line to parse.
  ...
  private static Event readEvent(String line)
  {
    ...
    try {
      ...
      event = new BagScannedEvent(eventData[0], tag, Location.valueOf(eventData[2].trim()),
          DATE_FORMAT.parse(eventData[3].trim()));
